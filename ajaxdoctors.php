...
$sql = "SELECT * FROM admins WHERE job='Médecin' AND del<>'yes'";
  ...
  if ($resulto->num_rows > 0) {
    while($rowo = $resulto->fetch_assoc()) {   
      if("$row[ID]" == "$rowo[ID]"){
?>
["<?php echo "$row[ID]"; ?>", "<img src='img/admin-<?php echo "$row[gender]"; ?>.png' style='width: 45px; height: 45px; border-radius: 50%;'>", "<?php echo "$row[name]"; ?>", "<?php echo "$row[fn]"; ?>", "<?php echo "$row[dob]"; ?>", "<?php echo "$row[gender]"; ?>", "<?php echo "$row[email]"; ?>", "<?php echo "$row[pn]"; ?>", "<?php echo "$row[wilaya]"; ?>", "<?php echo "$row[address]"; ?>", "<?php echo "$row[description]"; ?>", "<?php echo "$row[groupage]"; ?>", "<img id='<?php echo "$row[ID]"; ?>f' style='margin-left: 2.5px;' src='<?php if("$row[seal]" != ""){echo "$row[seal]";}else{echo "img/no.jpg";} ?>' width='50' height='50'>", "<img id='<?php echo "$row[ID]"; ?>s' style='margin-left: 2.5px;' src='<?php if("$row[signature]" != ""){ echo "$row[signature]";}else{echo "img/no.jpg";} ?>' width='50' height='50'>", "<span id='acdesac' class='badge badge-pill acdesac <?php if("$row[status]" == "Activé"){ echo "badge-success"; }else{ echo "badge-danger"; } ?>'><?php echo "$row[status]"; ?></span>", "<?php echo "$row[code]"; ?>", "<div class='ui buttons'><a class='edit ui button green icon' href='#edit'><i class='edit icon'></i></a><a class='print ui button blue icon' href='#print'><i class='print icon'></i></a><a class='delete ui button red icon' href='#delete'><i class='trash icon'></i></a></div>"]
<?php
}else{
?>
["<?php echo "$row[ID]"; ?>", "<img src='img/admin-<?php echo "$row[gender]"; ?>.png' style='width: 45px; height: 45px; border-radius: 50%;'>", "<?php echo "$row[name]"; ?>", "<?php echo "$row[fn]"; ?>", "<?php echo "$row[dob]"; ?>", "<?php echo "$row[gender]"; ?>", "<?php echo "$row[email]"; ?>", "<?php echo "$row[pn]"; ?>", "<?php echo "$row[wilaya]"; ?>", "<?php echo "$row[address]"; ?>", "<?php echo "$row[description]"; ?>", "<?php echo "$row[groupage]"; ?>", "<img id='<?php echo "$row[ID]"; ?>f' style='margin-left: 2.5px;' src='<?php if("$row[seal]" != ""){echo "$row[seal]";}else{echo "img/no.jpg";} ?>' width='50' height='50'>", "<img id='<?php echo "$row[ID]"; ?>s' style='margin-left: 2.5px;' src='<?php if("$row[signature]" != ""){ echo "$row[signature]";}else{echo "img/no.jpg";} ?>' width='50' height='50'>", "<span id='acdesac' class='badge badge-pill acdesac <?php if("$row[status]" == "Activé"){ echo "badge-success"; }else{ echo "badge-danger"; } ?>'><?php echo "$row[status]"; ?></span>", "<?php echo "$row[code]"; ?>", "<div class='ui buttons'><a class='edit ui button green icon' href='#edit'><i class='edit icon'></i></a><a class='print ui button blue icon' href='#print'><i class='print icon'></i></a><a class='delete ui button red icon' href='#delete'><i class='trash icon'></i></a></div>"],
