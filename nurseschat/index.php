...
<a id="exit" class="ui blue tag label">Gestion des infirmiers</a>
...
$sql = "SELECT * FROM admins WHERE job='Infirmier' AND mp='$_COOKIE[name]' AND del<>'yes'";
}
...
<a class="item" id="<?php echo "$row[ID]" ?>" onclick="show(<?php echo "$row[ID]" ?>, '<?php echo "$row[name]" ?>', '<?php echo "$row[email]" ?>', '<?php echo "$row[gender]" ?>')" id="chatr">
        <img src="../img/admin-<?php echo "$row[gender]"; ?>.png" class="imgradius" style="width: 35px; height: 35px;"><span style="margin-left: 10px;"><?php echo "$row[name]"; ?></span>
</a>
<?php
}}
?>
